...
__asm __saveds int RIPEMD160_Init_AmiSSL(register __a0 RIPEMD160_CTX *c)
{
	return(RIPEMD160_Init(c));
}

__asm __saveds int RIPEMD160_Update_AmiSSL(register __a0 RIPEMD160_CTX *c, register __a1 const void *data, register __d0 unsigned long len)
{
	return(RIPEMD160_Update(c, data, len));
}

__asm __saveds int RIPEMD160_Final_AmiSSL(register __a0 unsigned char *md, register __a1 RIPEMD160_CTX *c)
{
