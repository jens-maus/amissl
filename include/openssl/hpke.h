...
#   define OSSL_HPKE_CTX_new(mode,suite,role,libctx,propq) \
  IAmiSSL->OSSL_HPKE_CTX_new_amiga(mode,&(suite),role,libctx,propq)
#  else
#   define OSSL_HPKE_CTX_new(mode,suite,role,libctx,propq) \
  OSSL_HPKE_CTX_new_amiga(mode,&(suite),role,libctx,propq)
#  endif
# else
...
# if defined(OPENSSL_SYS_AMIGA) && !defined(AMISSL_COMPILE)
#  if defined(__amigaos4__) && !defined(__USE_INLINE__)
#   define OSSL_HPKE_keygen(suite,pub,publen,priv,ikm,ikmlen,libctx,propq) \
  IAmiSSL->OSSL_HPKE_keygen_amiga(&(suite),pub,publen,priv,ikm,ikmlen,libctx,propq)
#  else
#   define OSSL_HPKE_keygen(suite,pub,publen,priv,ikm,ikmlen,libctx,propq) \
  OSSL_HPKE_keygen_amiga(&(suite),pub,publen,priv,ikm,ikmlen,libctx,propq)
#  endif
# else
...
# if defined(OPENSSL_SYS_AMIGA) && !defined(AMISSL_COMPILE)
#  if defined(__amigaos4__) && !defined(__USE_INLINE__)
#   define OSSL_HPKE_suite_check(suite) IAmiSSL->OSSL_HPKE_suite_check_amiga(&(suite))
#  else
#   define OSSL_HPKE_suite_check(suite) OSSL_HPKE_suite_check_amiga(&(suite))
#  endif
# else
...
# if defined(OPENSSL_SYS_AMIGA) && !defined(AMISSL_COMPILE)
#  if defined(__amigaos4__) && !defined(__USE_INLINE__)
#   define OSSL_HPKE_get_ciphertext_size(suite,clearlen) IAmiSSL->OSSL_HPKE_get_ciphertext_size_amiga(&(suite),clearlen)
#   define OSSL_HPKE_get_public_encap_size(suite) IAmiSSL->OSSL_HPKE_get_public_encap_size_amiga(&(suite))
#   define OSSL_HPKE_get_recommended_ikmelen(suite) IAmiSSL->OSSL_HPKE_get_recommended_ikmelen_amiga(&(suite))
#  else
#   define OSSL_HPKE_get_ciphertext_size(suite,clearlen) OSSL_HPKE_get_ciphertext_size_amiga(&(suite),clearlen)
#   define OSSL_HPKE_get_public_encap_size(suite) OSSL_HPKE_get_public_encap_size_amiga(&(suite))
#   define OSSL_HPKE_get_recommended_ikmelen(suite) OSSL_HPKE_get_recommended_ikmelen_amiga(&(suite))
#  endif
# else
