...
#define X509_aux_print(out, x, indent) IAmiSSL->X509_aux_print((out), (x), (indent))
...
#define SSL_CTX_get0_alpn_protos(ctx, protos, protos_len) IAmiSSL->SSL_CTX_get0_alpn_protos((ctx), (protos), (protos_len))
#define SSL_get0_alpn_protos(ssl, protos, protos_len) IAmiSSL->SSL_get0_alpn_protos((ssl), (protos), (protos_len))
#define SSL_set1_dnsname(s, dnsname) IAmiSSL->SSL_set1_dnsname((s), (dnsname))
#define SSL_add1_dnsname(s, dnsname) IAmiSSL->SSL_add1_dnsname((s), (dnsname))
#define SSL_set1_ipaddr(s, ipaddr) IAmiSSL->SSL_set1_ipaddr((s), (ipaddr))
#define SSL_add1_ipaddr(s, ipaddr) IAmiSSL->SSL_add1_ipaddr((s), (ipaddr))
#define SSL_get_peer_addr(ssl, peer_addr) IAmiSSL->SSL_get_peer_addr((ssl), (peer_addr))
#define SSL_CTX_is_quic(c) IAmiSSL->SSL_CTX_is_quic((c))
#define SSL_CTX_is_server(c) IAmiSSL->SSL_CTX_is_server((c))
#define OSSL_QUIC_method() IAmiSSL->OSSL_QUIC_method()
#define SSL_listen_ex(listener, new_conn) IAmiSSL->SSL_listen_ex((listener), (new_conn))
#define OSSL_ECHSTORE_new(libctx, propq) IAmiSSL->OSSL_ECHSTORE_new((libctx), (propq))
#define OSSL_ECHSTORE_free(es) IAmiSSL->OSSL_ECHSTORE_free((es))
#define OSSL_ECHSTORE_new_config_amiga(es, echversion, max_name_length, public_name, suite) IAmiSSL->OSSL_ECHSTORE_new_config_amiga((es), (echversion), (max_name_length), (public_name), (suite))
#define OSSL_ECHSTORE_write_pem(es, index, out) IAmiSSL->OSSL_ECHSTORE_write_pem((es), (index), (out))
#define OSSL_ECHSTORE_read_echconfiglist(es, in) IAmiSSL->OSSL_ECHSTORE_read_echconfiglist((es), (in))
#define OSSL_ECHSTORE_get1_info(es, index, loaded_secs, public_name, echconfig, has_private, for_retry) IAmiSSL->OSSL_ECHSTORE_get1_info((es), (index), (loaded_secs), (public_name), (echconfig), (has_private), (for_retry))
#define OSSL_ECHSTORE_downselect(es, index) IAmiSSL->OSSL_ECHSTORE_downselect((es), (index))
#define OSSL_ECHSTORE_set1_key_and_read_pem(es, priv, in, for_retry) IAmiSSL->OSSL_ECHSTORE_set1_key_and_read_pem((es), (priv), (in), (for_retry))
#define OSSL_ECHSTORE_read_pem(es, in, for_retry) IAmiSSL->OSSL_ECHSTORE_read_pem((es), (in), (for_retry))
#define OSSL_ECHSTORE_num_keys(es, numkeys) IAmiSSL->OSSL_ECHSTORE_num_keys((es), (numkeys))
#define OSSL_ECHSTORE_flush_keys(es, age) IAmiSSL->OSSL_ECHSTORE_flush_keys((es), (age))
#define SSL_CTX_set1_echstore(ctx, es) IAmiSSL->SSL_CTX_set1_echstore((ctx), (es))
#define SSL_set1_echstore(s, es) IAmiSSL->SSL_set1_echstore((s), (es))
#define SSL_CTX_get1_echstore(ctx) IAmiSSL->SSL_CTX_get1_echstore((ctx))
#define SSL_get1_echstore(s) IAmiSSL->SSL_get1_echstore((s))
#define SSL_ech_get1_status(s, inner_sni, outer_sni) IAmiSSL->SSL_ech_get1_status((s), (inner_sni), (outer_sni))
#define SSL_ech_set_grease_type(s, type) IAmiSSL->SSL_ech_set_grease_type((s), (type))
#define SSL_ech_set_callback(s, f) IAmiSSL->SSL_ech_set_callback((s), (f))
#define SSL_CTX_ech_set_callback(ctx, f) IAmiSSL->SSL_CTX_ech_set_callback((ctx), (f))
#define OSSL_ECHSTORE_num_entries(es, numentries) IAmiSSL->OSSL_ECHSTORE_num_entries((es), (numentries))
#define SSL_ech_set1_server_names(s, inner_name, outer_name, no_outer) IAmiSSL->SSL_ech_set1_server_names((s), (inner_name), (outer_name), (no_outer))
#define SSL_ech_set1_outer_server_name(s, outer_name, no_outer) IAmiSSL->SSL_ech_set1_outer_server_name((s), (outer_name), (no_outer))
#define SSL_ech_set1_outer_alpn_protos(s, protos, protos_len) IAmiSSL->SSL_ech_set1_outer_alpn_protos((s), (protos), (protos_len))
#define SSL_ech_set1_grease_suite(s, suite) IAmiSSL->SSL_ech_set1_grease_suite((s), (suite))
#define SSL_ech_get1_retry_config(s, ec, eclen) IAmiSSL->SSL_ech_get1_retry_config((s), (ec), (eclen))
#define SSL_CTX_ech_set1_outer_alpn_protos(s, protos, protos_len) IAmiSSL->SSL_CTX_ech_set1_outer_alpn_protos((s), (protos), (protos_len))
#define SSL_set1_ech_config_list(ssl, ecl, ecl_len) IAmiSSL->SSL_set1_ech_config_list((ssl), (ecl), (ecl_len))
#define SSL_get0_sigalg(s, idx, codepoint, name) IAmiSSL->SSL_get0_sigalg((s), (idx), (codepoint), (name))
#define SSL_get0_shared_sigalg(s, idx, codepoint, name) IAmiSSL->SSL_get0_shared_sigalg((s), (idx), (codepoint), (name))
#define EC_GROUP_security_bits(group) IAmiSSL->EC_GROUP_security_bits((group))
#define ASN1_BIT_STRING_get_length(abs, length, unused_bits) IAmiSSL->ASN1_BIT_STRING_get_length((abs), (length), (unused_bits))
#define BIO_set_send_flags(b, flags) IAmiSSL->BIO_set_send_flags((b), (flags))
#define BIO_eof(b) IAmiSSL->BIO_eof((b))
#define CRYPTO_atomic_store_int(dst, val, lock) IAmiSSL->CRYPTO_atomic_store_int((dst), (val), (lock))
#define X509_check_certificate_times(vpm, x, error) IAmiSSL->X509_check_certificate_times((vpm), (x), (error))
#define X509_VERIFY_PARAM_set1_host_input_validation(param, validate_host) IAmiSSL->X509_VERIFY_PARAM_set1_host_input_validation((param), (validate_host))
#define X509_VERIFY_PARAM_set1_rfc822(param, email, emaillen) IAmiSSL->X509_VERIFY_PARAM_set1_rfc822((param), (email), (emaillen))
#define X509_VERIFY_PARAM_add1_rfc822(param, email, len) IAmiSSL->X509_VERIFY_PARAM_add1_rfc822((param), (email), (len))
#define X509_VERIFY_PARAM_set1_rfc822_input_validation(param, validate_rfc822) IAmiSSL->X509_VERIFY_PARAM_set1_rfc822_input_validation((param), (validate_rfc822))
#define X509_VERIFY_PARAM_set1_smtputf8(param, email, emaillen) IAmiSSL->X509_VERIFY_PARAM_set1_smtputf8((param), (email), (emaillen))
#define X509_VERIFY_PARAM_add1_smtputf8(param, email, len) IAmiSSL->X509_VERIFY_PARAM_add1_smtputf8((param), (email), (len))
#define X509_VERIFY_PARAM_set1_smtputf8_input_validation(param, validate_smtputf8) IAmiSSL->X509_VERIFY_PARAM_set1_smtputf8_input_validation((param), (validate_smtputf8))
#define X509_VERIFY_PARAM_add1_ip(param, ip, len) IAmiSSL->X509_VERIFY_PARAM_add1_ip((param), (ip), (len))
#define X509_VERIFY_PARAM_add1_ip_asc(param, ipasc) IAmiSSL->X509_VERIFY_PARAM_add1_ip_asc((param), (ipasc))
#define X509_VERIFY_PARAM_set1_ip_input_validation(param, validate_ip) IAmiSSL->X509_VERIFY_PARAM_set1_ip_input_validation((param), (validate_ip))
#define OSSL_STORE_INFO_new_SKEY(skey) IAmiSSL->OSSL_STORE_INFO_new_SKEY((skey))
#define OSSL_STORE_INFO_get0_SKEY(info) IAmiSSL->OSSL_STORE_INFO_get0_SKEY((info))
#define OSSL_STORE_INFO_get1_SKEY(info) IAmiSSL->OSSL_STORE_INFO_get1_SKEY((info))
#define OPENSSL_posix_to_tm(time, out_tm) IAmiSSL->OPENSSL_posix_to_tm((time), (out_tm))
#define OPENSSL_tm_to_posix(tm, out) IAmiSSL->OPENSSL_tm_to_posix((tm), (out))
#define OPENSSL_timegm(tm, out) IAmiSSL->OPENSSL_timegm((tm), (out))
#define OSSL_PARAM_clear_free(p) IAmiSSL->OSSL_PARAM_clear_free((p))
#define CMS_dataFinal_ex(cms, bio, data) IAmiSSL->CMS_dataFinal_ex((cms), (bio), (data))
#define CMS_SignerInfo_verify_ex(si, chain, data) IAmiSSL->CMS_SignerInfo_verify_ex((si), (chain), (data))
#define EVP_SIGNATURE_has_message_update(sig) IAmiSSL->EVP_SIGNATURE_has_message_update((sig))
#define EVP_MD_CTX_serialize(ctx, out, outlen) IAmiSSL->EVP_MD_CTX_serialize((ctx), (out), (outlen))
#define EVP_MD_CTX_deserialize(ctx, in, inlen) IAmiSSL->EVP_MD_CTX_deserialize((ctx), (in), (inlen))
#define OSSL_ENCODER_CTX_ctrl_string(ctx, name, val) IAmiSSL->OSSL_ENCODER_CTX_ctrl_string((ctx), (name), (val))
#define OPENSSL_sk_set_cmp_thunks(st, c_thunk) IAmiSSL->OPENSSL_sk_set_cmp_thunks((st), (c_thunk))
#define ASN1_BIT_STRING_set1(abs, data, length, unused_bits) IAmiSSL->ASN1_BIT_STRING_set1((abs), (data), (length), (unused_bits))
#define OSSL_ESS_check_signing_certs_ex(ss, ssv2, chain, libctx, propq, require_signing_cert) IAmiSSL->OSSL_ESS_check_signing_certs_ex((ss), (ssv2), (chain), (libctx), (propq), (require_signing_cert))
#define X509v3_delete_extension(x, loc) IAmiSSL->X509v3_delete_extension((x), (loc))
